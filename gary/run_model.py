...
def run_training(x, y, model, feature, out_dir='trained_results'):
    ...
    if model == 'ngboost':
        ...
        for i, tr_, va_, te_ in zip(range(len(train)), train, val, test):
            ...
            x_scaler = QuantileTransformer(n_quantiles=int(x_train.shape[0]/2.0)) if feature in ['mordred', 'pca_mordred'] else FunctionTransformer()
        ...
        for i, tr_, va_, te_ in zip(range(len(train)), train, val, test):
            ...
            for epoch in range(n_epoch):
                ...
                print(f'Epoch {epoch:<4} Loss: {val_avg_loss} Best loss: {best_loss} Time elapsed: {time.time()-s_time}')
