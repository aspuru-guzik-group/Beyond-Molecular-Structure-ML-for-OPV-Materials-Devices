...
from opv_ml.ML_models.pytorch.Catalysis_Hein.data.tokenizer import Tokenizer
from opv_ml.ML_models.pytorch.Catalysis_Hein.LR.opv_LR import LRModel
from opv_ml.ML_models.pytorch.Catalysis_Hein.NN.opv_NN import NNModel
from opv_ml.ML_models.pytorch.LSTM.opv_LSTM import LSTMModel

from opv_ml.ML_models.pytorch.Catalysis_Hein.data.data import OPVDataModule

import os

from opv_ml.ML_models.pytorch.Transformer.opv_chembert import TransformerModel as TFModel
...
#     "opv_ml", "data/postprocess/frag_master_opv_ml_from_min_old.csv"
# )
...
MODEL_CHECKPOINT = pkg_resources.resource_filename("opv_ml", "models/checkpoints")
...
class Evaluator:
    ...
    def model_eval(self, input_rep):
        ...
        if self.model_type == "LR":
            model = LRModel.load_from_checkpoint(final_model)
    ...
    if model == "LSTM":
        ...
        if linear:
            ...
            working_eval = Evaluator(
                ...
                MODEL_CHECKPOINT,
                PREDICTION_PATH,
                "aug_smi_ChemBERT-epoch=20-val_loss_mse=0.032-drop_prob=0.2-lr=0.01-train_batch_size=128.ckpt",
                DATA_DIR,
            )
    else:
        working_eval = Evaluator(
            ...
            MODEL_CHECKPOINT,
            PREDICTION_PATH,
            "smi_shuffled_NN-epoch=171-val_loss=0.055-n_hidden=256-n_embedding=128-drop_prob=0.3-lr=0.001-train_batch_size=128.ckpt",
            DATA_DIR,
        )
