...
opv_file: Path = Path.home() / 'Downloads' / 'FINAL Machine Learning OPV Parameters.xlsx'
...
filters: Dict[str, set] = {'fabrication': (refs | outputs | labels_molecules | labels_fabrication),
                           ...
                           'electrical': (refs | outputs | labels_molecules | labels_fabrication | labels_device | labels_electrical),
                           'fabrication_wo_solids': (refs | outputs | labels_molecules | labels_fabrication_wo_solids),
                           'device_wo_solids_thick': (refs | outputs | labels_molecules | labels_fabrication_wo_solids | labels_device_wo_thickness)}

# Get available subsets for ML
available: Dict[str, pd.DataFrame] = {'fabrication': opv_data[filters['fabrication']].dropna(),
...
# Data summary
data_summary: Dict[str, int] = {'data points': len(opv_data),
...
# Subset sizes + %
subset_summary: Dict[str, Union[int, float]] = {'fabrication (raw)': len(available['fabrication']),
                                                ...
                                                'fabrication w/o solids (%)': len(available['fabrication_wo_solids']) / data_summary['data points'],
                                                'device w/o solids or thickness (raw)': len(available['device_wo_solids_thick']),
                                                'device w/o solids or thickness (%)': len(available['device_wo_solids_thick']) / data_summary['data points']}
