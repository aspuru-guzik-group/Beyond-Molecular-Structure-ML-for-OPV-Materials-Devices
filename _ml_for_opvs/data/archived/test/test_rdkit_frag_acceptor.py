...
IMAGE_PATH = pkg_resources.resource_filename("_ml_for_opvs", "data/input_representation/test/")

# Learn ReactionFromSmarts
# print(rxn.RunReactants((Chem.MolFromSmiles("CCC(CC=O)CCC=O"),)))
# RunReactants returns all possible products, but does not apply reaction multiple times
# rxn_tuples = rxn.RunReactants((Chem.MolFromSmiles("CCC(CC=O)CCC=O"),))
# print(Chem.MolToSmiles(rxn.RunReactants(rxn_tuples[0])[0][0]))

# loop that will RunReactants until all functional groups are fragmented
# rxn = AllChem.ReactionFromSmarts(
#     "[C:1][C:2][c:3]:[c:4]:[c:5]:[c:6]:[c:7]>>[C:1][C:2][c:3].[c:4][c:6].[c:5][c:7]"
# )
# rxn = AllChem.ReactionFromSmarts(
#     "[C:1][C:2][c;R2;r5:3][c;R2;r5:4][c;R2;r5:5][c;R2;r5:6][c;R2;r5:7]>>[C:1][C:2][c:3].[c:4][c:6].[c:5][c:7]"
# )
# Oct 16 - figure out how to encode aromatic carbon w/ degree = 4
# rxn = AllChem.ReactionFromSmarts(
#     "[C;D4][c;R2;r5:4][c;R2;r5:5][c;R2;r5:6][c;R2;r5:7]>>[C;D4].[c:4][c:6].[c:5][c:7]"
# )
# mol = Chem.MolFromSmiles("CC7(C)c4cc3c2sc1ccsc1c2C(C)(C)c3cc4c6sc5ccsc5c67")
# product = rxn.RunReactants((mol,))
# print(len(product))
# for mol in product:
#     index = 0
#     while index < len(mol):
#         print(Chem.MolToSmiles(mol[index]))
#         index += 1

# if len(product) != 1:
#     while len(product) != 1:
#         product = rxn.RunReactants((product[0][0],))

# product_smi = Chem.MolToSmiles(product[0][0])
# print(product_smi)

# NOTE: Problems with Reaction SMARTS
# 1) No control over indexing
# 2) Cannot label broken bonds (unless we use atomic elements not present in dataset)
# 3) Hard to implement because it applies over the entire molecule
...
print("spacer: ", spacer_grp)
print("CORE: ", core_grp)
print("CORE FUNCTIONAL: ", core_func_grp)
print("CORE FUNCTIONAL IDX: ", core_func_atom_idx)

# total_grp = copy.copy(core_grp)
# total_grp.extend(spacer_grp)
# total_grp.extend(func_grp)
# total_grp.extend(core_func_grp)


# core_and_spacer_grp = copy.copy(spacer_grp)
# core_and_spacer_grp.extend(core_grp)
...
#     mol_index = 0
#     for mol_cycle in core_and_spacer_grp:  # NOTE: needs better name than mol_cycle
#         bonded_mol = False
...
#         #         ]
#         #         result_core_spacer = [
#         #             i for i in result_core_spacer if i != bond_to_cycle_idx
#         #         ]
#         #         fragment_bond_idx.extend(result_core_spacer)
...
#         if len(list(rw_atom.GetPropNames())) != 0:
