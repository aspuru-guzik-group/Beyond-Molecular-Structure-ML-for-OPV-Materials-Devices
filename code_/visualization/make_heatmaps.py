    ...
    for file in score_files:
        if not file.exists():
            ...
            if multioutput is not None:
                for target in multioutput:
                    avg = data[f"{score}_{target}_avg"]
                    if var == "stdev":
                        std = data[f"{score}_{target}_stdev"]
                    elif var == "stderr":
                        std = data[f"{score}_{target}_stderr"]
...
def create_fabrication_grid_heatmap(root_dir: Path, score: str, var: str) -> None:
    ...
    y_labels: list[str] = ["SVR", "RF", "HGB", "XGB", "NGB"][::-1]
    ...
    feature_labels: list[str] = [
        f"{rep}-{x_label}" for rep, x_label in product(representations, x_labels)
    ]
...
def create_subspace_grid_heatmap(root_dir: Path, score: str, var: str) -> None:
    ...
    # Create the product of representations and x_labels where they're joined as f"{rep}_{x_label}"
    feature_labels: list[str] = [
        f"{rep}-{space}" for rep, space in product(representations, spaces)
    ]
...
if __name__ == "__main__":
    ...
    results = root / "results" / f"target_PCE"

    for score in ["r", "r2", "rmse", "mae"]:
        # Model-representation grid
